    ...
    for row in 0..rows {
        for col in 0..columns {
            ...
            let is_in_selection = if let (Some((start_col, start_row)), Some((end_col, end_row))) =
                (tileset_manager.selection_start, tileset_manager.selection_end) {
    ...
    if let Some((tile_id, col, row)) = hovered_tile {
        let tooltip_text = if tileset_manager.selected_tiles.len() > 1 {
            format!("Tile ID: {}\nGrid: ({}, {})\nStamp: {} tiles selected",
                tile_id, col, row, tileset_manager.selected_tiles.len())
        } else {
            format!("Tile ID: {}\nGrid: ({}, {})\nDrag to select multiple", tile_id, col, row)
        };
