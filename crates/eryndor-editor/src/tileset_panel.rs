...
use crate::tileset_manager::{TilesetManager, LoadTilesetEvent};
        ...
        .show(ctx, |ui| {
            ...
            if ui.button("📁 Load Tileset...").clicked() {
                    ...
                    .pick_file()
                {
                    ...
                    load_events.send(LoadTilesetEvent {
            ...
            if tileset_manager.tilesets.is_empty() {
                ...
                egui::ScrollArea::vertical().show(ui, |ui| {
                    for (id, tileset_info) in tileset_manager.tilesets.iter() {
                        ...
                        ui.push_id(id, |ui| {
                            if ui.selectable_label(is_selected, &tileset_info.data.identifier).clicked() {
                            ...
                            if is_selected {
                                ...
                                ui.label(format!("Tile Size: {}x{}px",
                                    tileset_info.data.tile_width,
                                    tileset_info.data.tile_height
                                ));
                                ui.label(format!("Grid: {}x{} tiles",
                                    tileset_info.data.columns,
                                    tileset_info.data.rows
                                ));
                                ui.label(format!("Total Tiles: {}", tileset_info.tile_count));

                                // Show image dimensions if loaded
                                if let Some(image) = images.get(&tileset_info.texture_handle) {
                                    ui.label(format!("Image: {}x{}px",
                                        image.width(),
                                        image.height()
                                    ));
                                    ...
                                    if image_width % tileset_info.data.tile_width != 0 {
                                        ui.colored_label(
                                            egui::Color32::YELLOW,
                                            "⚠ Width not evenly divisible"
                                        );
                                    }
                                    if image_height % tileset_info.data.tile_height != 0 {
                                        ui.colored_label(
                                            egui::Color32::YELLOW,
...
fn render_tileset_grid(
    ...
    // Allocate space for the full tileset image
    ...
    if let Some((tile_id, col, row)) = hovered_tile {
        response.on_hover_text(format!("Tile ID: {}\nGrid: ({}, {})", tile_id, col, row));
    }
