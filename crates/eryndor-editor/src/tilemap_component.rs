...
pub fn sync_tilemap_entities(
    ...
    mut tilemap_query: Query<(Entity, &mut TilemapComponent, &Transform), Changed<TilemapComponent>>,
    ...
    for (entity, mut tilemap_comp, transform) in tilemap_query.iter_mut() {
        ...
        if let Some(tileset_id) = tilemap_comp.tileset_id {
            if let Some(tileset_info) = tileset_manager.tilesets.get(&tileset_id) {
                info!("Creating tilemap entity for TilemapComponent on entity {:?}", entity);
                ...
                info!("Created tilemap entity: {:?} for TilemapComponent", tilemap_entity);
            }
        }
    }
}
