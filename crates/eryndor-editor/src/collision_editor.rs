...
use eryndor_common::{CollisionShape, TileCollisionData, Vector2};

use crate::tileset_manager::TilesetManager;

/// Current tool in the collision editor
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CollisionTool {
    Select,      // Select and move shapes
    Rectangle,   // Draw rectangles
    Ellipse,     // Draw ellipses
    Polygon,     // Draw polygons point-by-point
    Polyline,    // Draw polylines point-by-point
    Point,       // Place points
}

/// State for the collision editor
#[derive(Resource)]
...
pub fn collision_editor_ui(
    ...
    egui::Window::new("Collision Editor")
        ...
        .show(ctx, |ui| {
            ...
            ui.label("Tool:");
            ui.horizontal(|ui| {
                if ui.selectable_label(collision_editor.current_tool == CollisionTool::Select, "Select").clicked() {
                    collision_editor.current_tool = CollisionTool::Select;
                }
                if ui.selectable_label(collision_editor.current_tool == CollisionTool::Rectangle, "Rect").clicked() {
                    collision_editor.current_tool = CollisionTool::Rectangle;
                }
                if ui.selectable_label(collision_editor.current_tool == CollisionTool::Ellipse, "Ellipse").clicked() {
                    collision_editor.current_tool = CollisionTool::Ellipse;
                }
            });
            ui.horizontal(|ui| {
                if ui.selectable_label(collision_editor.current_tool == CollisionTool::Polygon, "Polygon").clicked() {
                    collision_editor.current_tool = CollisionTool::Polygon;
                }
                if ui.selectable_label(collision_editor.current_tool == CollisionTool::Polyline, "Polyline").clicked() {
                    collision_editor.current_tool = CollisionTool::Polyline;
                }
                if ui.selectable_label(collision_editor.current_tool == CollisionTool::Point, "Point").clicked() {
            ...
            // Tile selection
            if let (Some(tileset_id), Some(tile_id)) = (tileset_manager.selected_tileset_id, tileset_manager.selected_tile_id) {
                ui.label(format!("Editing Tile: {} (Tileset {})", tile_id, tileset_id));
            ...
            egui::ScrollArea::vertical().max_height(200.0).show(ui, |ui| {
                for (idx, shape) in collision_editor.shapes.iter().enumerate() {
                    ui.horizontal(|ui| {
                        ...
                        if ui.selectable_label(current_selection == Some(idx), shape_name).clicked() {
            ...
            match collision_editor.current_tool {
                ...
                CollisionTool::Rectangle | CollisionTool::Ellipse => ui.label("Click and drag to create shape"),
                CollisionTool::Polygon | CollisionTool::Polyline => ui.label("Click to add points, double-click to finish"),
...
pub fn handle_collision_input(
    ...
    match collision_editor.current_tool {
        ...
        CollisionTool::Polygon | CollisionTool::Polyline => {
            ...
            if mouse_button.just_pressed(MouseButton::Left) {
                collision_editor.polygon_points.push(Vector2::new(world_pos.x, world_pos.y));
            }

            // Double click (or right click) finishes polygon/polyline
            if mouse_button.just_pressed(MouseButton::Right) && !collision_editor.polygon_points.is_empty() {
                let shape = match collision_editor.current_tool {
...
pub fn render_collision_shapes(
    mut gizmos: Gizmos,
    collision_editor: Res<CollisionEditor>,
) {
    if !collision_editor.active {
        return;
    }
    ...
    for (idx, shape) in collision_editor.shapes.iter().enumerate() {
        ...
        match shape {
            CollisionShape::Rectangle { x, y, width, height } => {
                gizmos.rect_2d(
                    Isometry2d::new(Vec2::new(*x + width / 2.0, *y + height / 2.0), Rot2::default()),
    ...
    if matches!(collision_editor.current_tool, CollisionTool::Polygon | CollisionTool::Polyline) {
